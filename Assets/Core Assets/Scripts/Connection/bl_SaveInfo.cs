...
public class bl_SaveInfo : MonoBehaviour {
    ...
    [Header("Global")]
    public string SaveInfoPHP_Url = "http://127.0.0.1/MyGame/Login/bl_SaveInfo.php";
    ...
    /// <param name="d"></param>
    /// <param name="s"></param>
    void GetInfo(string n, int k, int d, int s,int st,string ip)
    {
        m_UserName = n;
        m_Kills = k;
        m_Deaths = d;
    ...
    IEnumerator SendUpdateIP()
    {
        ...
        WWW w = new WWW(SaveInfoPHP_Url, wf);
        ...
        else
        {
            if(w.text == "successip")
            {
                Debug.Log(string.Format("Save new IP{0} succefull!",m_IP));
            }
            else
            {
                Debug.Log(w.text);
